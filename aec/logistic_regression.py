...
방법:
  ...
  - 다변량 (Multivariate): 전체 변수 동시 투입 → Adjusted OR + 모델 요약
...
def run_multivariate(df: pd.DataFrame) -> tuple:
    """
    전체 예측변수 동시 투입 다중 로지스틱 회귀.
...
def main():
    ...
    # ── 다변량 ──────────────────────────────────────────────────────────────
    print("\n[다변량 분석]")
    coef_df, summary = run_multivariate(df)

    sig = coef_df[coef_df['p_value'] < 0.05]
    print(f"\n  ▶ 유의한 계수 (p<0.05, {len(sig)}/{len(coef_df)}개 — 전체 테이블은 Excel 참조):")
    print(sig[['Variable', 'Adj_OR', 'CI_Lower', 'CI_Upper', 'p_value']].to_string(index=False))

    print("\n  ▶ 모델 성능:")
    for k in ['N', 'N_events', 'AUC_mean', 'AUC_CI_lower', 'AUC_CI_upper',
              'Sensitivity', 'Specificity', 'PPV', 'NPV',
              'HL_stat', 'HL_p', 'HL_result',
              'Nagelkerke_R2', 'Brier_score', 'AIC', 'BIC']:
        print(f"    {k:25s}: {summary[k]}")

    if not summary.get('converged', True):
        print("\n  ⚠ 경고: 모델 수렴 실패. 결과 해석 주의.")
