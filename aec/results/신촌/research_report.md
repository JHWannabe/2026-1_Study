...
| Case 3 | 성별, 나이, AEC 특징 + **CT 모델명** (ManufacturerModelName) |
...
|------|---|------|----|--------|-----|-----|
| 남성 (M) | 649 | 149.03 | 26.62 | 149.00 | 132.00 | 166.00 |
| 여성 (F) | 648 | 104.46 | 16.02 | 103.00 | 95.00 | 114.00 |
...
| ManufacturerModelName | One-hot encoding (drop_first=True) → 46개 더미 변수 |
...
|------|---|-------|---------|-----|
| Sex (M=1, F=0) | 43.7066 * | [41.3257, 46.0876] | 0.00e+00 | 0.5004 |
| Age (표준화) | -2.1201 * | [-3.8004, -0.4399] | 0.013 | 0.0047 |
| AEC: p25 (표준화) | 7.3803 * | [5.7449, 9.0158] | 0.00e+00 | 0.0571 |
| AEC: CV (표준화) | -3.7464 * | [-5.4181, -2.0746] | 1.20e-05 | 0.0147 |
| AEC: skewness (표준화) | -1.0374 | [-2.7207, 0.6458] | 0.227 | 0.0011 |
| AEC: slope_abs_mean (표준화) | 3.1148 * | [1.4392, 4.7905] | 2.76e-04 | 0.0102 |
| AEC: mean (표준화) | 7.0355 * | [5.3955, 8.6754] | 0.00e+00 | 0.0519 |
| ManufacturerModelName (F-test) | N/A (범주형) * | N/A | 0.0093 | 0.0547 |
...
|------|----|------|
| N | 1297 | 총 환자 수 |
| R² | **0.5814** | TAMA 분산의 58.1% 설명 |
| Adjusted R² | **0.5636** | 변수 수 페널티 적용 설명력 |
| F-statistic | 32.579 (p=3.17e-196) | 전체 모델 유의성 |
| RMSE | **19.987 cm²** | 예측 오차 (임상 단위) |
| MAE | 14.688 cm² | 이상치 강건 오차 |
| AIC | 11558.01 | 모델 복잡도 대비 적합도 |
| BIC | 11837.08 | 강화된 복잡도 페널티 |
...
|------|--------|---------|------|------|
| Shapiro-Wilk | 0.9384 | 0.00e+00 | 비정규 | 잔차 정규성 |
| Breusch-Pagan | 100.5504 | 8.80e-05 | 이분산 | 등분산성 |
| Durbin-Watson | 2.0268 | — | 정상 | 자기상관 (1.5-2.5 정상) |
| Condition number (κ) | 379.4 | — | 주의(κ<1000) | 다중공선성 |
...
|------|----------|-------|---------|-----|
| Sex (M=1, F=0) | 1.0756 | [0.8353, 1.385] | 0.572 | 0.5091 |
| Age (표준화) | 1.4364 * | [1.2482, 1.6531] | 0.00e+00 | 0.614 |
| AEC: p25 (표준화) | 0.7019 * | [0.6036, 0.8163] | 4.00e-06 | 0.594 |
| AEC: CV (표준화) | 1.1002 | [0.9718, 1.2455] | 0.131 | 0.5368 |
| AEC: skewness (표준화) | 0.9912 | [0.8744, 1.1236] | 0.890 | 0.5157 |
| AEC: slope_abs_mean (표준화) | 0.7514 | [0.53, 1.0652] | 0.108 | 0.5533 |
| AEC: mean (표준화) | 0.6945 * | [0.5991, 0.8051] | 1.00e-06 | 0.5981 |
| ManufacturerModelName (LR χ²-test) | LR_χ²=70.594 * | N/A | 0.011 | 0.602 |
...
| 지표 | 값 | 근거 |
|------|----|------|
| N | 1297 (양성: 319, 24.6%) | 총 환자 수 |
| AUC-ROC | **0.7180** [0.6868–0.7494] | 판별 능력 (Bootstrap 95%CI) |
| Sensitivity | 0.6520 | TP/(TP+FN) |
| Specificity | 0.6759 | TN/(TN+FP) |
| PPV | 0.3962 | 양성 예측도 |
| NPV | 0.8562 | 음성 예측도 |
| Hosmer-Lemeshow | χ²=6.283, p=0.616 | 보정도 (p>0.05 = 보정 양호) |
| Nagelkerke R² | **0.1713** | 로지스틱 설명력 (pseudo-R²) |
| Brier Score | 0.1620 | 확률 예측 정밀도 (0=완벽) |
| AIC | 1396.34 | 모델 복잡도 대비 적합도 |
...
|------|--------|--------|--------|--------------|--------------|
| R² | 0.5183 | 0.5455 | 0.5814 | **+0.0272** | +0.0359 |
| Adj R² | 0.5176 | 0.543 | 0.5636 | +0.0254 | +0.0206 |
| RMSE (cm²) | 21.4418 | 20.8283 | 19.9873 | -0.6135 | -0.8410 |
| AIC | 11638.23 | 11572.92 | 11558.01 | -65.31 | -14.91 |

#### 4.3.2 로지스틱 회귀 성능 비교

| 지표 | Case 1 | Case 2 | Case 3 | Case 1→2 향상 | Case 2→3 향상 |
|------|--------|--------|--------|--------------|--------------|
| AUC | 0.614 | 0.6546 | 0.718 | **+0.0406** | +0.0634 |
| Nagelkerke R² | 0.0314 | 0.0754 | 0.1713 | +0.0440 | +0.0959 |
| AIC | 1425.39 | 1395.6 | 1396.34 | -29.79 | 0.74 |
| HL p | 1.2160e-03 | 1.7660e-01 | 6.1556e-01 | — | — |
...
2. **AEC 특징의 독립적 기여:** AEC feature 추가(Case 1→2) 시 선형 R²가 **0.518 → 0.545** (Δ+0.027)으로 향상됨. 로지스틱 AUC는 **0.614 → 0.655** (Δ+0.041).

3. **CT 모델명의 추가 기여:** 모델명 추가(Case 2→3) 시 선형 R²가 추가로 +0.036, 로지스틱 AUC +0.063 향상. AIC 감소로 모델 적합도 개선 확인.

4. **최종 모델 (Case 3):** 선형 R²=0.5814, RMSE=19.9873 cm², 로지스틱 AUC=0.718 [0.6868-0.7494].
