...
add_text(slide,
         "데이터셋: 강남 CT  |  N=1,673명  |  Python (statsmodels · scikit-learn)",
         1.3, 4.7, 10.7, 0.5,
...
add_table(slide,
          ...
          [
              ...
              ["Case 3", "Case 2 + CT 모델명 (ManufacturerModelName — 30개 dummy) + KVP"],
          ],
...
add_table(slide,
          ...
          [
              ...
              ["여성 (F)", "1,008", "104.69", "15.71", "103.00", "95.00", "114.00"],
          ],
...
add_text(slide,
         "• 남성: TAMA < 132 cm²  →  Low Muscle (1)\n"
         "• 여성: TAMA < 95 cm²  →  Low Muscle (1)\n"
         "• 양성 비율: 400 / 1,673 = 23.9 %",
...
add_table(slide,
          ...
          [
              ...
              ["mean", "+0.297", "AEC 평균값 → 체격 전반 반영", "확인됨", "✔ (포함)"],
...
add_table(slide,
          ...
          [
              ["Sex (M=1, F=0)", "44.84 *", "[42.76, 46.91]", "< 0.001", "0.518"],
              ["Age (표준화)", "-2.99 *", "[-4.45, -1.54]", "< 0.001", "0.010"],
              ["AEC: p25 (표준화)", "11.17 *", "[9.81, 12.53]", "< 0.001", "0.134"],
              ["AEC: CV (표준화)", "-10.64 *", "[-12.01, -9.27]", "< 0.001", "0.122"],
              ["AEC: skewness (표준화)", "-10.47 *", "[-11.84, -9.09]", "< 0.001", "0.118"],
              ["AEC: slope_abs_mean (표준화)", "1.28", "[-0.19, 2.74]", "0.087", "0.002"],
              ["AEC: mean (표준화)", "9.05 *", "[7.65, 10.45]", "< 0.001", "0.088"],
              ["ManufacturerModelName", "N/A (범주형)*", "N/A", "6.0e-06", "0.045"],
          ],
...
add_table(slide,
          ["검정", "통계량", "p-value", "결과"],
          [
              ["Shapiro-Wilk", "0.936", "< 0.001", "비정규 ⚠"],
              ["Breusch-Pagan", "123.41", "< 0.001", "이분산 ⚠"],
              ["Durbin-Watson", "1.9456", "—", "정상 ✔"],
              ["Condition κ", "164.57", "—", "주의(κ<1,000) ✔"],
          ],
...
add_table(slide,
          ...
          [
              ["Sex (M=1, F=0)", "1.056", "[0.840, 1.328]", "0.639", "0.507"],
              ["Age (표준화)", "1.585 *", "[1.402, 1.792]", "< 0.001", "0.623"],
              ["AEC: p25 (표준화)", "0.588 *", "[0.519, 0.666]", "< 0.001", "0.645"],
              ["AEC: CV (표준화)", "1.118 *", "[1.000, 1.249]", "0.050", "0.541"],
              ["AEC: skewness (표준화)", "1.173 *", "[1.044, 1.319]", "0.0074", "0.544"],
              ["AEC: slope_abs_mean (표준화)", "1.029", "[0.920, 1.150]", "0.618", "0.495"],
              ["AEC: mean (표준화)", "0.586 *", "[0.520, 0.662]", "< 0.001", "0.653"],
              ["ManufacturerModelName", "LR χ²=50.59 *", "N/A", "0.011", "0.581"],
          ],
          0.35, 1.25, 12.6, 3.3, font_size=11)

add_text(slide, "* p < 0.05 유의  |  p25·mean: OR < 1 → 해당 변수↑ 시 Low TAMA 위험 감소  |  Sex: 이진화 후 단변량 비유의",
...
add_table(slide,
          ["지표", "값", "해석"],
          [
              ["Hosmer-Lemeshow χ²", "4.346", "p=0.825 → 보정도 양호 ✔"],
              ["Brier Score", "0.1556", "낮을수록 우수 (0=완벽, 0.25=무작위)"],
              ["AIC", "1,658.03", "Case 비교 기준"],
          ],
...
add_table(slide,
          ...
          [
              ...
              ["RMSE (cm²)", "20.43", "18.40", "17.83", "-2.04 ▼", "-0.57 ▼"],
              ["AIC", "14,849", "14,508", "14,463", "-341 ▼", "-45 ▼"],
          ],
...
add_table(slide,
          ["지표", "Case 1\n(Sex+Age)", "Case 2\n(+AEC)", "Case 3\n(+CT Model)",
           "Case1→2 Δ", "Case2→3 Δ"],
          [
              ["AUC-ROC", "0.624", "0.720", "0.7522", "+0.096 ▲▲", "+0.032 ▲"],
              ["Nagelkerke R²", "0.0516", "0.1596", "0.2145", "+0.108 ▲▲", "+0.055 ▲"],
              ["AIC", "1,788", "1,668", "1,658", "-120 ▼▼", "-10 ▼"],
              ["HL p-value", "1.26e-03", "0.816", "0.825", "보정도 개선 ✔", "보정도 양호 ✔"],
          ],
          0.35, 1.25, 12.6, 2.3, font_size=12)

add_text(slide,
         "AEC 추가(Case1→2)로 AUC +9.6%p 대폭 향상  |  CT 모델명 추가(Case2→3)로 +3.2%p 추가 개선",
         0.35, 3.65, 12.6, 0.45, size=13, bold=True, color=BLUE)
...
conclusions = [
    ...
    ("AEC 특징의 독립적 기여",
     "Case1→2  선형 R²  +0.085 (0.551→0.636)\n"
     "Case1→2  로지스틱 AUC  +0.096 (0.624→0.720)"),
    ("CT 모델명의 추가 기여",
     "Case2→3  선형 R²  +0.022  |  로지스틱 AUC  +0.032\n"
     "AIC 추가 감소 → 모델 적합도 개선 확인"),
    ("최종 모델 (Case 3) 요약",
     "선형: R²=0.6579, RMSE=17.83 cm²\n"
     "로지스틱: AUC=0.7522 [0.726–0.777]  |  HL p=0.825 (보정도 양호)"),
]
...
limits = [
    ...
    "Logistic 이진화 임계값 (M<132, F<95 cm²) — 문헌 기반, 기관별 참조값 확인 필요",
