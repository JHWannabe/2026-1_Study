...
OUT_DIR = Path(__file__).resolve().parent / "outputs" / "0706" / "aec_new_region_surrogate_combo_gate_with_r0"
...
def main() -> None:
    ...
    print("candidates per region:", {k: len(v) for k, v in by_region.items()}, flush=True)
    print("n_combos:", n_combos, flush=True)
    import os as _os

    if _os.environ.get("R0_PROBE_ONLY"):
        print("R0_PROBE_ONLY set - stopping before combo search.", flush=True)
        return
