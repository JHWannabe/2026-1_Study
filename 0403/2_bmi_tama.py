...
def configure_sex_filter(sex_filter: str) -> None:
    global SEX_FILTER, SEX_FILTER_DIR, RESULT_DIR, OUTPUT_PATH

    SEX_FILTER = sex_filter
    SEX_FILTER_DIR = SEX_FILTER_LABELS[sex_filter]
    RESULT_DIR = BASE_DIR / "result" / "0403" / SITE_NAME / SEX_FILTER_DIR
...
def load_bmi_tama_data(file_path: Path) -> pd.DataFrame:
    df = pd.read_excel(file_path, usecols=REQUIRED_COLUMNS).copy()
