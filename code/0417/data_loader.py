...
def load_raw_data() -> pd.DataFrame:
    ...
    """
    meta  = pd.read_excel(config.EXCEL_PATH, sheet_name='metadata')
...
def get_feature_cols(case: int, df: pd.DataFrame) -> list:
    ...
    Case 1: AEC features (표준화) — 성별·나이 제외
    Case 2: AEC features + Sex + Age_z — 성별·나이 추가
    Case 3: AEC features + Sex + Age_z + ManufacturerModelName 더미
    """
    base  = ['Sex', 'Age_z']
