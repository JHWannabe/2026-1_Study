...
입력: 연구코드/results/0529/liver_pubis_32/aec_inspection/aec_scaling_compare_aec128.xlsx
출력: 동일 폴더에
  aec_scaling_mean_curves.png         — 전체 128 위치
  aec_scaling_mean_curves_crop80.png  — 중앙 80% 구간 (양끝 10% 제거)

각 스케일링(raw / std_scaled / std_norm)에 대해
...
XLSX_PATH = os.path.join(_ROOT, "results", "0529", "liver_pubis_32",
                         "aec_inspection", "aec_scaling_compare_aec128.xlsx")
OUT_DIR = os.path.dirname(XLSX_PATH)

SHEET_LABELS = [
    ("raw",        "Raw AEC"),
    ("std_scaled", "StandardScaler (열 방향)"),
    ("std_norm",   "StandardScaler + 행 z-score"),
]
...
def _build_crop80_sheets() -> dict[str, pd.DataFrame]:
    """raw 시트에서 crop80 슬라이싱 후 표준화 두 버전을 반환."""
    ...
    X_raw = df_raw[crop_cols].values.astype(np.float64)
    X_std = StandardScaler().fit_transform(X_raw).astype(np.float32)
    X_norm = _row_normalize(X_std)

    sheets = {}
    for name, X in [("raw", X_raw), ("std_scaled", X_std), ("std_norm", X_norm)]:
...
def plot_mean_curves() -> None:
    ...
    fig, axes = plt.subplots(1, 3, figsize=(18, 5.75))
