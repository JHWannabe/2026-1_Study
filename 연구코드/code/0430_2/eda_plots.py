...
def run_tama_stats(df_clean: pd.DataFrame, BASE_DIR: Path, hosp_label: str) -> pd.DataFrame:
    """
    성별 SMI 기술통계 계산 → Excel + Fig 19 (히스토그램 패널) + Fig 20 (박스플롯) 저장.
    Returns: smi_stats_df
    """
    ...
    for sex_enc, sex_label in [(None, "전체"), (0, "여성(F)"), (1, "남성(M)")]:
        ...
        smi = sub["SMI"].dropna()
        smi_stats_rows.append({
            "그룹":   sex_label,
            "N":      len(smi),
            "Mean":   round(smi.mean(), 2),
            "SD":     round(smi.std(), 2),
            "Median": round(smi.median(), 2),
            "P25":    round(smi.quantile(0.25), 2),
            "P75":    round(smi.quantile(0.75), 2),
            "Min":    round(smi.min(), 2),
            "Max":    round(smi.max(), 2),
        })

    smi_stats_df = pd.DataFrame(smi_stats_rows)
    print(f"\n  [SMI 기술통계 - {hosp_label}]")
    print(smi_stats_df.to_string(index=False))
    smi_stats_df.to_excel(BASE_DIR / "smi_sex_stats.xlsx", index=False)
    print(f"  Saved: smi_sex_stats.xlsx")

    # ── Fig 19: SMI 히스토그램 패널 (전체 / 여성 / 남성) ──
    ...
    for ax, (sex_enc, sex_label) in zip(axes, [(None, "전체"), (0, "여성(F)"), (1, "남성(M)")]):
        ...
        p25_v    = smi.quantile(0.25)
        median_v = smi.median()
        mean_v   = smi.mean()

        ax.hist(smi, bins=35, color=color, alpha=0.7, edgecolor="white", linewidth=0.4)
        ...
        ax.set_xlabel("SMI (cm²/m²)", fontsize=10)
        ax.set_ylabel("환자 수", fontsize=10)
        ax.set_title(
            f"{sex_label}  (N={len(smi)})\n"
            f"Mean={mean_v:.1f}, SD={smi.std():.1f}, Median={median_v:.1f}\n"
            f"P25={p25_v:.1f}, P75={smi.quantile(0.75):.1f}",
            fontsize=10,
        )
        ax.legend(fontsize=8)
        ax.grid(axis="y", alpha=0.3)

    plt.suptitle(f"[{hosp_label}] 성별 SMI 분포 (정제 후 데이터 기준)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    fig.savefig(BASE_DIR / "19_smi_sex_distribution.png", dpi=150)
    plt.close()
    print(f"  Saved: 19_smi_sex_distribution.png")

    # ── Fig 20: 성별 SMI 박스플롯 ──
    fig, ax = plt.subplots(figsize=(7, 5))
    groups = {
        "여성(F)": df_clean[df_clean["PatientSex_enc"] == 0]["SMI"].dropna(),
        "남성(M)": df_clean[df_clean["PatientSex_enc"] == 1]["SMI"].dropna(),
    }
    bp = ax.boxplot(
        list(groups.values()),
    ...
    for i, (sex_label, smi) in enumerate(groups.items(), start=1):
        ax.scatter([i + 0.25] * len(smi), smi,
                   alpha=0.15, s=6, color=box_colors[i-1], zorder=3)
    ax.set_ylabel("SMI (cm²/m²)", fontsize=11)
    ax.set_title(f"[{hosp_label}] 성별 SMI 분포 박스플롯\n(점: 개별 환자)",
                 fontsize=12, fontweight="bold")
    ax.yaxis.grid(True, linestyle="--", alpha=0.4)
    ax.set_axisbelow(True)
    plt.tight_layout()
    fig.savefig(BASE_DIR / "20_smi_sex_boxplot.png", dpi=150)
    plt.close()
    print(f"  Saved: 20_smi_sex_boxplot.png")
...
def run_eda_figs(
    ...
    """
    EDA 그래프 생성.
    - Fig 01: AEC Feature-SMI Pearson 상관 Top 20
    - Fig 02: VIF (다중공선성 검사)
    - Fig 03: 성별 SMI 분포 및 이진화 임계값
    - Fig 16: CT 스캐너 모델 분포
    - Fig 17: kVp 분포
    ...
    # ── Fig 01: AEC Feature - SMI 상관 Top 20 ──
    non_aec_set = ({"PatientID", "PatientAge", "PatientSex", "PatientSex_enc",
                    "BMI", "ManufacturerModelName", "kVp", "SMI"} | set(MODEL_COLS))
    aec_all_cols = [c for c in df_eda.columns if c not in non_aec_set]
    corr_rows01  = []
    for f in aec_all_cols:
        sub = df_eda[[f, "SMI"]].dropna()
        if len(sub) < 10:
            continue
        r01, _ = scipy_stats.pearsonr(sub[f], sub["SMI"])
    ...
    if corr_rows01:
        ...
        ax.set_xlabel("Pearson r with SMI")
        ax.set_title(f"AEC Feature - SMI 상관계수 Top 20\n(붉은색=양, 파란색=음)")
    ...
    # ── Fig 03: 성별 SMI 분포 및 이진화 임계값 ──
    df_m03  = df_clean[df_clean["PatientSex_enc"] == 1]["SMI"].astype(float)
    df_f03  = df_clean[df_clean["PatientSex_enc"] == 0]["SMI"].astype(float)
    ...
    for ax, data, lbl_sex, color, thr in [
        ...
        ax.axvline(thr, color="#e74c3c", ls="--", lw=2, label=f"임계값 {thr:.2f}")
        ax.axvline(data.mean(), color="black", ls="-", lw=1.5, label=f"평균 {data.mean():.2f}")
        low_n = int((data < thr).sum())
        ax.set_xlabel("SMI"); ax.set_ylabel("환자 수")
        ax.set_title(f"{lbl_sex} SMI 분포\n(N={len(data)}, 중앙값={data.median():.2f})")
        ax.text(0.62, 0.92, f"임계값 이하: {low_n}명 ({low_n/len(data)*100:.1f}%)",
                transform=ax.transAxes, fontsize=9,
                bbox=dict(boxstyle="round", facecolor="lightyellow", alpha=0.8))
        ax.legend(fontsize=9)
    plt.suptitle(f"[{hosp_label}] 성별 SMI 분포 및 이진화 임계값",
                 fontsize=13, fontweight="bold")
    plt.tight_layout(); fig.savefig(BASE_DIR / "03_smi_distribution.png", dpi=150); plt.close()
    print(f"  Saved: 03_smi_distribution.png")
