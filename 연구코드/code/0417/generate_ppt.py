...
FIGURES = "c:/Users/user/Desktop/새 폴더/results/figures"
...
# ════════════════════════════════════════════════════════════
# 슬라이드 8 — 선형 회귀: 성별·나이 유무 비교
# ════════════════════════════════════════════════════════════
slide = prs.slides.add_slide(BLANK)
add_rect(slide, 0, 0, 13.33, 7.5, fill_rgb=GRAY)
slide_header(slide, "04  선형 회귀 — 성별·나이 유무 비교", "Model A (AEC only) vs Model B (AEC + Sex + Age)")

# 비교 테이블
add_table(slide,
          ["지표", "Model A\n(AEC+CT, 성별·나이 제외)", "Model B\n(AEC+성별+나이+CT)", "Δ (B−A)"],
          [
              ["R²",         "0.2625",         "0.6572",  "+0.395 ▲▲"],
              ["Adj R²",     "0.2472",         "0.6497",  "+0.402 ▲▲"],
              ["RMSE (cm²)", "26.17",          "17.84",   "−8.33 ▼▼"],
              ["MAE (cm²)",  "20.46",          "13.18",   "−7.28 ▼▼"],
              ["CV R²",      "0.221 ± 0.054",  "0.634 ± 0.025", "+0.413 ▲▲"],
              ["AIC",        "15,732",         "14,455",  "−1,277 ▼▼"],
          ],
          0.35, 1.25, 12.6, 2.55, font_size=12)

add_text(slide,
         "성별·나이 추가(Model B)로 R² +39.5%p, RMSE −8.3 cm², CV R² +41.3%p — 인구통계가 TAMA 예측의 핵심 기여 변수",
         0.35, 3.9, 12.6, 0.45, size=13, bold=True, color=BLUE)

# 잔차 진단 (Model B 기준)
add_text(slide, "잔차 진단 (Model B 기준)", 0.35, 4.45, 6.5, 0.35, size=13, bold=True, color=NAVY)
add_table(slide,
          ["검정", "통계량", "p-value", "결과"],
          [
              ["Shapiro-Wilk", "0.9697", "< 0.001", "비정규 ⚠"],
              ["Breusch-Pagan", "—", "< 0.001", "이분산 ⚠"],
              ["Durbin-Watson", "1.9654", "—", "정상 ✔"],
              ["Condition κ", "145.81", "—", "주의(κ<1,000) ✔"],
          ],
          0.35, 4.82, 6.5, 1.75, font_size=11)

add_text(slide,
         "⚠  비정규·이분산 → Robust Standard Errors 적용 계획",
         0.35, 6.65, 6.5, 0.5, size=11, italic=True, color=ORANGE)

# 실제 vs 예측 그림
add_text(slide, "실제 vs 예측 (Model B)", 7.1, 4.45, 5.5, 0.35, size=13, bold=True, color=NAVY)
add_image(slide, "04_linear_actual_vs_pred.png", 7.1, 4.82, 5.9, 2.45)
...
# ════════════════════════════════════════════════════════════
# 슬라이드 10 — 로지스틱 회귀: 성별·나이 유무 비교
# ════════════════════════════════════════════════════════════
slide = prs.slides.add_slide(BLANK)
add_rect(slide, 0, 0, 13.33, 7.5, fill_rgb=GRAY)
slide_header(slide, "05  로지스틱 회귀 — 성별·나이 유무 비교", "Model A (AEC only) vs Model B (AEC + Sex + Age)")

# 비교 테이블
add_table(slide,
          ["지표", "Model A\n(AEC+CT, 성별·나이 제외)", "Model B\n(AEC+성별+나이+CT)", "Δ (B−A)"],
          [
              ["AUC-ROC",       "0.7297",              "0.7794",              "+0.050 ▲"],
              ["95% CI",        "[0.703 – 0.756]",     "[0.756 – 0.802]",     "—"],
              ["Sensitivity",   "0.6017",              "0.7671",              "+0.165 ▲▲"],
              ["Specificity",   "0.7331",              "0.6674",              "−0.066"],
              ["Nagelkerke R²", "0.1925",              "0.2823",              "+0.090 ▲"],
              ["Brier Score",   "0.1769",              "0.1629",              "−0.014 ▼"],
              ["HL p-value",    "0.568 ✔",             "0.333 ✔",             "보정도 양호 유지"],
              ["AIC",           "1,845",               "1,723",               "−122 ▼"],
          ],
          0.35, 1.25, 12.6, 3.05, font_size=11)

add_text(slide,
         "AUC +5.0%p, 민감도 +16.5%p → 성별·나이 추가로 저근육량 선별 능력 대폭 향상  |  보정도(HL) 양호 유지",
         0.35, 4.38, 12.6, 0.45, size=13, bold=True, color=BLUE)

# Model A vs B KPI 강조 카드
kpi_cmp = [
    ("AUC\nModel A → B", "0.730 → 0.779"),
    ("민감도\nModel A → B", "0.602 → 0.767"),
    ("Nagelkerke R²\nModel A → B", "0.193 → 0.282"),
]
for i, (label, val) in enumerate(kpi_cmp):
    x = 0.35 + i * 4.2
    add_rect(slide, x, 4.95, 3.9, 1.3, fill_rgb=NAVY)
    add_text(slide, label, x+0.15, 4.98, 3.6, 0.5, size=10, bold=True, color=ORANGE)
    add_text(slide, val, x+0.15, 5.5, 3.6, 0.65, size=16, bold=True, color=WHITE)

# Calibration plot
add_text(slide, "Calibration Plot (Model B)", 0.35, 6.38, 5.5, 0.35, size=12, bold=True, color=NAVY)
add_image(slide, "09_logistic_calibration.png", 0.35, 6.75, 5.5, 0.55)
...
conclusions = [
    ...
    ("AEC만으로도 유의미한 예측 가능",
     "Model A (AEC+CT, 성별·나이 제외)\n"
     "선형 R²=0.263  |  로지스틱 AUC=0.730"),
    ("성별·나이 추가 시 성능 대폭 향상",
     "선형 R² +0.395 (0.263→0.657), RMSE −8.3 cm²\n"
     "로지스틱 AUC +0.050 (0.730→0.779), 민감도 +0.165"),
    ("최종 모델 (Model B) 요약",
...
# 저장
# ════════════════════════════════════════════════════════════
output_path = "c:/Users/user/Desktop/새 폴더/results/TAMA_연구보고서.pptx"
