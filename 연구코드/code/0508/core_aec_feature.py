...
def run_logistic_regression(target: str) -> None:
    ...
    df['Sex_Label'] = df['PatientSex'].map({0: 'Male', 1: 'Female'})
    _save_logistic_md(target, fold_acc, fold_auc,
                      test_acc, test_auc, test_cm, y_te, y_pred_te,
                      thresholds, coef_arr, pval_arr, df)
...
def _save_logistic_md(target, fold_acc, fold_auc,
                      test_acc, test_auc, test_cm, test_y_true, test_y_pred,
                      thresholds, coef, pvalues, df):
    df = df.copy()
...
# ── AEC 피처 선택 (fold별 호출) ───────────────────────────
def _select_aec_features(df_train: pd.DataFrame) -> list[str]:
    """
    ...
    with warnings.catch_warnings():
        ...
        while len(selected) > 1:
            ...
            if max(vifs) <= VIF_THRESHOLD:
                break
            selected.pop(int(np.argmax(vifs)))
...
def run_linear_regression_selected(target: str) -> None:
    ...
    try:
        ...
        for fold_i, (train_idx, val_idx) in enumerate(kf.split(df_tr), 1):
            ...
            fold_feats = _select_aec_features(fold_tr)
        ...
        # 최종 모델: train 전체로 학습 → test 평가
        final_feats = _select_aec_features(df_tr)
...
def run_logistic_regression_selected(target: str) -> None:
    ...
    try:
        ...
        df['Sex_Label'] = df['PatientSex'].map({0: 'Male', 1: 'Female'})
        _save_logistic_md_sel(target, fold_acc, fold_auc,
                              test_acc, test_auc, test_cm, y_te, y_pred_te,
                              thresholds, coef_arr, pval_arr, df, fold_n_sel)
    finally:
        RESULTS_DIR = _orig_dir
...
def _save_logistic_md_sel(target, fold_acc, fold_auc,
                          test_acc, test_auc, test_cm, test_y_true, test_y_pred,
                          thresholds, coef, pvalues, df, fold_n_sel):
    df = df.copy()
    avg_n = np.mean(fold_n_sel)
    feat_line = (f'AEC 선택 피처 (|r|<{CORR_THRESHOLD}, VIF<{VIF_THRESHOLD}) '
                 f'— Train 기준 {len(FEATURES)}개, 폴드 평균 {avg_n:.1f}개')
