...
from config import N_FOLDS, SEED, DEVICE, EPOCHS, LR, BATCH_SIZE, PATIENCE, AEC_NORM, AEC_OUT, AEC_FEAT_DIM
from models import SegmentFusionModel
...
def _make_loader(X_aec, X_clinic, y, shuffle):
    X_t = torch.tensor(X_aec,   dtype=torch.float32)   # (N, 60)
...
def _train_model(X_aec_tr, X_clinic_tr, y_tr, X_aec_val, X_clinic_val, y_val):
    ...
    model = SegmentFusionModel(
        aec_feat_dim=AEC_FEAT_DIM, n_clinic=X_clinic_tr.shape[1], aec_out=AEC_OUT
    ).to(DEVICE)
...
def compute_gradient_saliency(model, X_aec_s, X_clinic_s, batch_size=32):
    """Gradient saliency (|∂logit/∂input|) over 60-dim AEC segment features.

    Returns saliency: (N, 60) np.ndarray, max-normalized per sample.
    """
    model.eval()
    all_sal = []
    ...
    for start in range(0, len(X_aec_s), batch_size):
        X_b = torch.tensor(
            X_aec_s[start:start + batch_size], dtype=torch.float32
        ).to(DEVICE)
        C_b = torch.tensor(
        ...
        grads = torch.autograd.grad(logits.sum(), X_b)[0]  # (B, 60)
        sal = grads.abs().detach().cpu().numpy()

        mx = sal.max(axis=1, keepdims=True)
        mx[mx == 0] = 1.0
        all_sal.append(sal / mx)

    return np.concatenate(all_sal, axis=0)  # (N, 60)
