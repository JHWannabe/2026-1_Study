...
> **AEC 사용 변수:** mean, CV, skewness, slope_abs_mean
...
| Case | 예측변수 | 목적 |
|------|----------|------|
| Case 0 (AEC 단독) | **AEC 특징** (mean, CV, skewness, slope_abs_mean) | AEC만의 독립 진단력 |
| Case 1 | 성별 (Sex), 나이 (Age) | 인구통계 기반선 |
| Case 2 | 성별, 나이 + **AEC 특징** (mean, CV, skewness, slope_abs_mean) | AEC 기여도 정량화 |
...
|------|---|------|----|--------|-----|-----|
| 남성 (M) | 637 | 148.00 | 26.56 | 147.00 | 131.00 | 165.00 |
...
|---------|-----------|------|-----|
| **mean** | +0.234 | AEC 평균값 → 전체 tube current 수준 | 1.12 |
| **CV** | -0.126 | 변동계수 (std/mean) → 체형 불균일성 반영 | 1.27 |
| **skewness** | -0.044 | AEC 곡선 비대칭성 → 체형 분포 특성 | 1.20 |
| **slope_abs_mean** | +0.102 | 평균 절대 기울기 → 곡선 동역학 | 1.10 |

> **주의:** `mean`과 `AUC_normalized`는 동시 투입 시 VIF > 50,000으로 심각한 다중공선성이 확인된다. Amplitude 그룹(`mean`, `AUC_normalized`, `p25`, `peak_max_height`)에서 `mean` 하나만 선택하였다.
...
|------|---|-------|---------|-----|
| Sex (M=1, F=0) | 43.6693 * | [41.2733, 46.0653] | 0.00e+00 | 0.5023 |
| Age (표준화) | -2.3078 * | [-4.0011, -0.6145] | 0.0076 | 0.0056 |
| AEC: mean (표준화) | 7.1988 * | [5.5478, 8.8499] | 0.00e+00 | 0.0546 |
| AEC: CV (표준화) | -3.874 * | [-5.5585, -2.1894] | 7.00e-06 | 0.0158 |
| AEC: skewness (표준화) | -1.3569 | [-3.0534, 0.3395] | 0.117 | 0.0019 |
| AEC: slope_abs_mean (표준화) | 3.1413 * | [1.4521, 4.8305] | 2.75e-04 | 0.0104 |
| ManufacturerModelName (F-test) | N/A (범주형) * | N/A | 0.0052 | 0.057 |
...
|------|----|------|
| N | 1269 | 총 환자 수 |
| R² | **0.5856** | TAMA 분산의 58.6% 설명 |
| Adjusted R² | **0.5682** | 변수 수 페널티 적용 설명력 |
| F-statistic | 33.720 (p=2.95e-195) | 전체 모델 유의성 |
| RMSE | **19.833 cm²** | 예측 오차 (임상 단위) |
| MAE | 14.633 cm² | 이상치 강건 오차 |
| AIC | 11287.17 | 모델 복잡도 대비 적합도 |
| BIC | 11554.76 | 강화된 복잡도 페널티 |
...
|------|--------|---------|------|------|
| Shapiro-Wilk | 0.9793 | 2.00e-06 | 비정규 | 잔차 정규성 |
| Breusch-Pagan | 118.7735 | 0.00e+00 | 이분산 | 등분산성 |
| Durbin-Watson | 0.9959 | — | 자기상관 주의 | 자기상관 (1.5-2.5 정상) |
| Condition number (κ) | 305.62 | — | 주의(κ<1000) | 다중공선성 |
...
| AUC-ROC | **0.7170** [0.6850–0.7487] | 판별 능력 (Bootstrap 95%CI) |
| Sensitivity | 0.6940 | TP/(TP+FN) |
| Specificity | 0.6324 | TN/(TN+FP) |
| PPV | 0.3860 | 양성 예측도 |
| NPV | 0.8612 | 음성 예측도 |
| Hosmer-Lemeshow | χ²=5.249, p=0.731 | 보정도 (p>0.05 = 보정 양호) |
| Nagelkerke R² | **0.1681** | 로지스틱 설명력 (pseudo-R²) |
| Brier Score | 0.1638 | 확률 예측 정밀도 (0=완벽) |
| AIC | 1377.83 | 모델 복잡도 대비 적합도 |
...
|------|-------------------|------------------|---------------|------------------|--------------|--------------|
| R² | 0.074 | 0.5203 | 0.548 | 0.5901 | **+0.0277** | +0.0421 |
| Adj R² | 0.071 | 0.5195 | 0.5459 | 0.5726 | +0.0264 | +0.0267 |
| RMSE (cm²) | 29.6475 | 21.3386 | 20.7131 | 19.7243 | -0.6255 | -0.9888 |
| AIC | 12213.51 | 11374.86 | 11307.35 | 11275.2 | -67.51 | -32.15 |

#### 4.3.2 로지스틱 회귀 성능 비교

| 지표 | Case 0 (AEC 단독) | Case 1 (Sex+Age) | Case 2 (+AEC) | Case 3 (+CT/kVp) | Case 1→2 향상 | Case 2→3 향상 |
|------|-------------------|------------------|---------------|------------------|--------------|--------------|
| AUC | 0.6117 | 0.6097 | 0.6497 | 0.7282 | **+0.0400** | +0.0785 |
| Nagelkerke R² | 0.0369 | 0.0286 | 0.0727 | 0.1791 | +0.0441 | +0.1064 |
| AIC | 1404.66 | 1407.95 | 1376.83 | 1369.12 | -31.12 | -7.71 |
| HL p | 3.2965e-01 | 6.2300e-04 | 1.6756e-01 | 1.8895e-01 | — | — |
...
2. **AEC 단독 진단력 (Case 0):** 성별·나이 없이 AEC 특징만으로 선형 R²=0.074, 로지스틱 AUC=0.612. AEC 신호가 독립적인 근육량 정보를 내포함을 시사.

3. **AEC 특징의 추가 기여 (Case 1→2):** AEC feature 추가 시 선형 R²가 **0.520 → 0.548** (Δ+0.028)으로 향상됨. 로지스틱 AUC는 **0.610 → 0.650** (Δ+0.040).

4. **CT 모델명·kVp의 추가 기여 (Case 2→3):** 모델명/kVp 추가 시 선형 R²가 추가로 +0.042, 로지스틱 AUC +0.078 향상. AIC 감소로 모델 적합도 개선 확인.

5. **최종 모델 (Case 3):** 선형 R²=0.5901, RMSE=19.7243 cm², 로지스틱 AUC=0.7282 [0.6986-0.7591].
