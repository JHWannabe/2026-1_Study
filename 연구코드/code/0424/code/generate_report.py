...
출력: results/{SITE}/research_report.md
"""
...
from feature_selection import scanner_distribution, kvp_distribution, compute_correlations, compute_vif
import pandas as pd
...
def generate_report():
    ...
    # AEC feature 상관계수 & VIF 동적 계산
    corr_df_full = compute_correlations(df_raw)
    corr_lookup = dict(zip(corr_df_full['Feature'], corr_df_full['Pearson_r']))

    df_enc_for_vif = data_loader.encode_sex(df_raw.copy())
    vif_cols = ['PatientAge', 'Sex'] + [f for f in aec_selected if f in df_enc_for_vif.columns]
    vif_df_feat = compute_vif(df_enc_for_vif[vif_cols].dropna(), vif_cols)
    vif_lookup = dict(zip(vif_df_feat['Feature'], vif_df_feat['VIF']))
...
`feature_selection.py`를 통해 TAMA와의 Pearson/Spearman 상관계수를 계산하고, VIF 검사로 다중공선성을 확인하여 아래 {len(aec_selected)}개 feature를 선택하였다.

{_aec_feature_table(aec_selected, corr_lookup, vif_lookup)}
...
_FEATURE_DESC = {
    'mean':          'AEC 평균값 → 전체 tube current 수준',
    'AUC_normalized':'정규화된 AUC → 전체 노출량 지표',
    ...
    'peak_max_height':'최대 피크 높이 → 최고 tube current',
    'peak_mean_height':'평균 피크 높이 → 평균 tube current',
    'kurtosis':      '첨도 → 곡선 분포 뾰족함',
    'std':           '표준편차 → tube current 변동 크기',
}


def _aec_feature_table(features, corr_lookup, vif_lookup):
    rows = ["| Feature | Pearson r | 해석 | VIF |",
            "|---------|-----------|------|-----|"]
    for feat in features:
        r_val = corr_lookup.get(feat, 'N/A')
        v_val = vif_lookup.get(feat, 'N/A')
        try:
            r_fmt = f"{float(r_val):+.3f}"
        except (ValueError, TypeError):
            r_fmt = str(r_val)
        try:
            v_fmt = f"{float(v_val):.2f}"
        except (ValueError, TypeError):
            v_fmt = str(v_val)
        desc = _FEATURE_DESC.get(feat, feat)
        rows.append(f"| **{feat}** | {r_fmt} | {desc} | {v_fmt} |")
    return '\n'.join(rows)
