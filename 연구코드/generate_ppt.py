...
def txt(s, text, l, t, w, h, sz=13, bold=False, color=C_DARK,
...
def hdr(s, title, sub=None):
    box(s, 0, 0, 13.33, 1.05, fill=C_DARK)
    txt(s, title, 0.35, 0.08, 11.0, 0.55, sz=25, bold=True, color=C_WHT)
    if sub: txt(s, sub, 0.35, 0.63, 11.0, 0.35, sz=13, color=C_MGR)
...
card(s, 0.3, 1.2, 5.9, 5.65)
txt(s, "연구 배경", 0.5, 1.3, 5.5, 0.35, sz=16, bold=True)
hline(s, 0.5, 1.67, 5.5, color=C_BLUE)
for i,t in enumerate([
    ...
    txt(s, t, 0.5, 1.77+i*0.9, 5.6, 0.82, sz=13.5, color=C_DARK)

card(s, 6.5, 1.2, 6.5, 5.65)
txt(s, "비교 연구 목적 (DeLong Test 기반)", 6.7, 1.3, 6.1, 0.35, sz=16, bold=True)
hline(s, 6.7, 1.67, 6.1, color=C_TEAL)
for i,(q,a,col) in enumerate([
    ...
    box(s, 6.6, 1.77+i*1.25, 0.08, 1.05, fill=col)
    txt(s, q, 6.75, 1.79+i*1.25, 6.1, 0.35, sz=13, bold=True, color=col)
    txt(s, a, 6.75, 2.14+i*1.25, 6.1, 0.55, sz=12, color=C_DARK)
snum(s, 2)
...
card(s, 0.3, 1.2, 5.9, 6.1)
txt(s, "데이터셋 구성", 0.5, 1.28, 5.5, 0.33, sz=15, bold=True)
hline(s, 0.5, 1.63, 5.5, color=C_BLUE)
hx = [0.5, 1.6, 3.1, 4.6]; hw = [1.05, 1.45, 1.45, 1.5]
trow_h(s, ["구분","남성(M)","여성(F)","합계"], hx, hw, 1.68)
for i,(row,fill) in enumerate(zip([
    ...
    trow_d(s, row, hx, hw, 2.0+i*0.37, ht=0.37, fill=fill)
txt(s, "연령: 57.6±12.1세  |  BMI: 23.6±3.3 kg/m²  |  Train:Test = 8:2",
    0.5, 2.76, 5.5, 0.28, sz=11, color=C_MGR, italic=True)

txt(s, "AEC 전처리 조건 (5종)", 0.5, 3.18, 5.5, 0.33, sz=15, bold=True)
hline(s, 0.5, 3.53, 5.5, color=C_TEAL)
for i,(k,v) in enumerate([
    ...
    txt(s, k,  0.5, 3.62+i*0.4, 1.7, 0.38, sz=12, bold=True, color=C_BLUE)
    txt(s, v,  2.2, 3.62+i*0.4, 3.9, 0.38, sz=12, color=C_DARK)
txt(s, "✕  BCE / Focal Loss  ×  AEC 128pt / 256pt  =  4 시나리오 × 5조건 = 20조건",
    0.5, 5.26, 5.5, 0.3, sz=11, color=C_MGR, italic=True)

card(s, 6.5, 1.2, 5.9, 3.1)
txt(s, "모델 구성", 6.7, 1.28, 5.5, 0.33, sz=15, bold=True)
hline(s, 6.7, 1.63, 5.5, color=C_ORG)
for i,(t,b,col) in enumerate([
    ...
    box(s, 6.6, 1.72+i*1.0, 0.08, 0.82, fill=col)
    txt(s, t, 6.75, 1.74+i*1.0, 5.4, 0.33, sz=13, bold=True, color=col)
    txt(s, b, 6.75, 2.07+i*1.0, 5.4, 0.28, sz=12, color=C_DARK)

pic(s, img(M1, "data_distribution.png"), 6.4, 4.45, 6.8, 2.7)
txt(s, "데이터 분포 (Train / Test / Sex / Sarcopenia)", 6.4, 4.38, 6.8, 0.28,
...
for i,(col,title,body) in enumerate([
    ...
    card(s, 0.3+i*4.35, 1.2, 4.15, 5.3)
    box(s, 0.3+i*4.35, 1.2, 4.15, 0.5, fill=col)
    txt(s, title, 0.4+i*4.35, 1.23, 3.95, 0.45, sz=15, bold=True, color=C_WHT)
    y = 1.85
    for line in body.split("\n"):
        txt(s, f"  {line}", 0.4+i*4.35, y, 3.9, 0.44, sz=13, color=C_DARK)
        y += 0.47
box(s, 0.3, 6.58, 12.7, 0.52, fill=RGBColor(0xEE,0xF5,0xFF))
txt(s, "유의수준:  *** p<0.001   ** p<0.01   * p<0.05   † p<0.10   ns p≥0.10",
    0.5, 6.61, 12.3, 0.42, sz=13, bold=True, color=C_DARK, align=PP_ALIGN.CENTER)
snum(s, 4)
...
for i,(row,fill,sc) in enumerate(zip(rows_ov,fills,sigs)):
    ...
    trow_d(s, row, hx, hw, 1.54+i*0.4, ht=0.4, fill=fill, colors=table_c,
           bolds=[i in [1,3]]*9)

# Left bottom: ROC comparison
txt(s, "ROC 비교 (M1·M2·M2₂·M3)", 0.3, 4.08, 4.5, 0.3, sz=12, bold=True)
pic(s, img(CMP128, "roc_all_models_norm.png"), 0.3, 4.4, 4.5, 2.9)

# Center bottom: key comparison highlights
card(s, 5.0, 4.08, 4.1, 3.22, fill=RGBColor(0xE8,0xF8,0xE8))
box(s, 5.0, 4.08, 0.1, 3.22, fill=C_GRN)
txt(s, "비교 핵심", 5.2, 4.16, 3.7, 0.32, sz=14, bold=True, color=C_GRN)
for i,t in enumerate([
    ...
    txt(s, t, 5.2, 4.57+i*0.45, 3.7, 0.42, sz=12, color=C_DARK)

# Right bottom: calibration (Brier shown → mandatory)
calib_banner(s, 9.25, 4.08, 3.85, "M3 norm  (Brier 0.1952)")
pic(s, img(M3N, "calibration.png"), 9.25, 4.43, 3.85, 2.87)
snum(s, 5)
...
# DeLong table (left)
card(s, 0.3, 1.2, 6.0, 4.1)
txt(s, "DeLong Test — M1(LR) vs M2(CrossAttn)  [ norm 조건 ]",
    0.5, 1.28, 5.6, 0.33, sz=14, bold=True)
hline(s, 0.5, 1.63, 5.6, color=C_BLUE)
dx = [0.45, 2.05, 3.1, 4.15, 5.2]; dw = [1.55, 1.0, 1.0, 1.0, 0.95]
trow_h(s, ["시나리오","M1 AUC","M2 AUC","Δ AUC","p-val"], dx, dw, 1.68)
dlong_m1m2 = [
...
for i,(row,sc) in enumerate(zip(dlong_m1m2, sig_c_m1m2)):
    ...
    trow_d(s, row, dx, dw, 2.0+i*0.4, ht=0.4, fill=fill,
           colors=[C_DARK,C_DARK,C_BLUE,C_BLUE,sc])

# Bootstrap CI (left continued)
txt(s, "Bootstrap 95% CI — BCE+AEC128+norm 기준",
    0.5, 3.7, 5.6, 0.3, sz=12, bold=True, color=C_DARK)
hline(s, 0.5, 4.02, 5.6, color=C_TEAL, h=0.03)
bx2 = [0.45, 1.85, 3.25, 4.7]; bw2 = [1.35, 1.35, 1.4, 1.25]
trow_h(s, ["지표","M1","M2","Δ(M2-M1)"], bx2, bw2, 4.08, ht=0.28)
for i,(row,fill) in enumerate(zip([
    ...
    trow_d(s, row, bx2, bw2, 4.38+i*0.33, ht=0.33, fill=fill)

# Right top: 4-panel ROC (all 4 scenarios)
txt(s, "조건별 ROC 비교  (norm 기준, M1·M2·M2₂·M3 포함)", 6.5, 1.2, 6.6, 0.3, sz=12, bold=True)
for j,(folder, lbl) in enumerate([
    (CMP128,"BCE+AEC128"), (CMP256,"BCE+AEC256"),
    (CMPF128,"Focal+AEC128"), (CMPF256,"Focal+AEC256"),
]):
    col_j = j%2; row_j = j//2
    x0 = 6.5 + col_j*3.4; y0 = 1.55 + row_j*1.72
...
# Right bottom: calibration (Brier shown → mandatory)
calib_banner(s, 6.5, 4.95, 6.6, "M2  BCE+AEC128+norm  (Brier 0.2124)")
pic(s, img(M2N, "calibration.png"), 6.5, 5.3, 6.6, 2.0)
snum(s, 6)
...
txt(s, "DeLong Test — M1(LR) vs M3(CrossAttn3)  [ norm 조건 ]",
    0.5, 1.28, 5.6, 0.33, sz=14, bold=True)
hline(s, 0.5, 1.63, 5.6, color=C_ORG)
trow_h(s, ["시나리오","M1 AUC","M3 AUC","Δ AUC","p-val"], dx, dw, 1.68)
dlong_m1m3 = [
...
txt(s, "Bootstrap 95% CI — BCE+AEC128+norm 기준",
    0.5, 3.7, 5.6, 0.3, sz=12, bold=True)
hline(s, 0.5, 4.02, 5.6, color=C_ORG, h=0.03)
trow_h(s, ["지표","M1","M3","Δ(M3-M1)"], bx2, bw2, 4.08, ht=0.28)
for i,(row,fill) in enumerate(zip([
...
calib_banner(s, 6.5, 4.95, 6.6, "M3  BCE+AEC128+norm  (Brier 0.1952)")
pic(s, img(M3N, "calibration.png"), 6.5, 5.3, 6.6, 2.0)
snum(s, 7)
...
# DeLong table
card(s, 0.3, 1.2, 7.5, 4.5)
txt(s, "DeLong Test — M2(CrossAttn) vs M3(CrossAttn3)  [ 전 조건 ]",
    0.5, 1.28, 7.1, 0.33, sz=14, bold=True)
hline(s, 0.5, 1.63, 7.1, color=C_MGR)
mx = [0.45, 2.3, 3.45, 4.6, 5.75, 6.85]
mw = [1.8,  1.1, 1.1,  1.1, 1.05, 0.9]
trow_h(s, ["조건","M2 AUC","M3 AUC","Δ AUC","p-val","sig"], mx, mw, 1.68)
m2m3_rows = [
...
for i,row in enumerate(m2m3_rows):
    fill = C_WHT if i%2==0 else C_LGR
    is_sig = row[-1] != "ns"
    trow_d(s, row, mx, mw, 2.0+i*0.37, ht=0.37, fill=fill,
           colors=[C_DARK]*4+[C_GRN if is_sig else C_MGR, C_GRN if is_sig else C_MGR])

# Right: ROC + insight
txt(s, "ROC — BCE+AEC128+norm  (M2 vs M3 근소 차이)", 8.0, 1.2, 5.1, 0.3, sz=12, bold=True)
pic(s, img(CMP128,"roc_all_models_norm.png"), 8.0, 1.52, 5.1, 2.68)

card(s, 8.0, 4.35, 5.1, 2.9, fill=RGBColor(0xF5,0xF5,0xF5))
box(s, 8.0, 4.35, 0.1, 2.9, fill=C_MGR)
txt(s, "해석", 8.2, 4.43, 4.7, 0.33, sz=14, bold=True, color=C_MGR)
for i,t in enumerate([
    ...
    txt(s, t, 8.2, 4.85+i*0.38, 4.7, 0.36, sz=11.5, color=C_DARK)
snum(s, 8)
...
# Experiment design
card(s, 0.3, 1.2, 4.8, 1.45)
txt(s, "실험 설계", 0.5, 1.28, 4.4, 0.3, sz=14, bold=True)
hline(s, 0.5, 1.6, 4.4, color=C_TEAL)
for i,t in enumerate([
    "M2: 환자 i의 Clinic + 환자 i의 AEC (동일 환자)",
    "M2_2: 환자 i의 Clinic + 환자 j의 AEC (다른 환자)",
    "→ 동일 CrossAttention 구조 / AEC만 랜덤 셔플",
]):
    txt(s, t, 0.5, 1.7+i*0.32, 4.4, 0.3, sz=12, color=C_DARK)

# DeLong table
card(s, 0.3, 2.78, 7.5, 4.0)
txt(s, "DeLong Test — M2(Matched) vs M2_2(Unmatched)", 0.5, 2.86, 7.1, 0.3, sz=14, bold=True)
hline(s, 0.5, 3.18, 7.1, color=C_TEAL)
nx = [0.45, 2.5, 3.7, 5.0, 5.95, 6.95]
nw = [2.0, 1.15, 1.25, 0.9, 0.95, 0.9]
trow_h(s, ["조건","M2 AUC","M2_2 AUC","Δ AUC","p-val","sig"], nx, nw, 3.23)
neg_ctrl = [
...
for i,row in enumerate(neg_ctrl):
    ...
    trow_d(s, row, nx, nw, 3.55+i*0.37, ht=0.37, fill=fill,
           colors=[C_DARK]*4+[sc,sc])

# Right: ROC most dramatic + insight
txt(s, "ROC — excl_extreme 조건 (가장 극적인 차이)",
    8.0, 1.2, 5.1, 0.3, sz=12, bold=True)
pic(s, img(CMP128,"roc_all_models_excl_extreme.png"), 8.0, 1.52, 5.1, 2.55)

card(s, 8.0, 4.2, 5.1, 2.58, fill=RGBColor(0xFF,0xF3,0xE0))
box(s, 8.0, 4.2, 0.1, 2.58, fill=C_ORG)
txt(s, "해석", 8.2, 4.28, 4.7, 0.3, sz=14, bold=True, color=C_ORG)
for i,t in enumerate([
    ...
    txt(s, t, 8.2, 4.66+i*0.38, 4.7, 0.36, sz=11.5, color=C_DARK)
snum(s, 9)
...
for (folder,label,note,col),(x0,y0) in zip(panels,[(0.25,1.2),(6.82,1.2),(0.25,4.3),(6.82,4.3)]):
    ...
    pic(s, img(folder,"roc_all_models_norm.png"), x0, y0+0.72, 6.3, 3.0)
snum(s, 10)
...
for j,(mdir,label,col) in enumerate([
    ...
    x0 = 0.3 + j*6.5
    box(s, x0, 1.2, 6.3, 0.38, fill=col)
    txt(s, label, x0+0.1, 1.22, 6.1, 0.34, sz=14, bold=True, color=C_WHT)
    txt(s, "Attention Heatmap (샘플별)", x0+0.1, 1.63, 3.0, 0.25, sz=10, color=C_MGR)
    pic(s, img(mdir,"attention_heatmap.png"), x0, 1.9, 6.2, 2.6)
    txt(s, "Attention Map C→A (평균)", x0+0.1, 4.55, 3.0, 0.25, sz=10, color=C_MGR)
    pic(s, img(mdir,"attention_map_c2a.png"), x0, 4.82, 6.2, 2.55)
snum(s, 11)
...
# Summary highlight
box(s, 0.3, 1.2, 12.7, 0.65, fill=RGBColor(0xE8,0xF8,0xE8))
box(s, 0.3, 1.2, 0.12, 0.65, fill=C_GRN)
txt(s,
    "BCE+AEC128+norm: M1→M3 p=0.040 (*)  /  "
    "BCE+AEC256+norm: M1→M2 p=0.013 (*) · M2 vs M2_2 p=0.005 (**)  "
    "→ 두 조건 모두 AEC 기여 통계 검증",
    0.55, 1.27, 12.1, 0.52, sz=13, bold=True, color=C_DARK)
...
for i,(row,fill) in enumerate(zip([
    ...
    trow_d(s, row, hx7, hw7, 2.27+i*0.4, ht=0.4, fill=fill,
           colors=[C_DARK]*6+[s6,s7,C_MGR], bolds=[i in [1,3]]*9)

# Bottom: ROC comparison + confusion + calibration (Brier → mandatory)
txt(s, "ROC — BCE+AEC128+norm", 0.3, 4.0, 3.9, 0.28, sz=11, bold=True)
pic(s, img(CMP128,"roc_all_models_norm.png"), 0.3, 4.3, 3.9, 2.95)

txt(s, "M3 Confusion Matrix", 4.35, 4.0, 3.9, 0.28, sz=11, bold=True)
pic(s, img(M3N,"confusion_matrices.png"), 4.35, 4.3, 3.9, 2.95)

calib_banner(s, 8.4, 4.0, 4.7, "M3  BCE+AEC128+norm  (Brier 0.1952)")
pic(s, img(M3N,"calibration.png"), 8.4, 4.35, 4.7, 2.9)
snum(s, 12)
...
for i,(row,fill) in enumerate(zip([
    ...
    trow_d(s, row, sx, sw, 1.57+i*0.41, ht=0.41, fill=fill, bolds=[i>0]*9)

# Analysis cards
card(s, 0.3, 3.3, 3.8, 3.85, fill=RGBColor(0xE8,0xF0,0xFF))
box(s, 0.3, 3.3, 0.1, 3.85, fill=C_BLUE)
txt(s, "남성 비교", 0.5, 3.38, 3.5, 0.32, sz=14, bold=True, color=C_BLUE)
for i,t in enumerate([
    ...
    txt(s, t, 0.5, 3.8+i*0.52, 3.6, 0.5, sz=12, color=C_DARK)

card(s, 4.25, 3.3, 3.8, 3.85, fill=RGBColor(0xFF,0xF0,0xF8))
box(s, 4.25, 3.3, 0.1, 3.85, fill=C_RED)
txt(s, "여성 비교", 4.45, 3.38, 3.5, 0.32, sz=14, bold=True, color=C_RED)
for i,t in enumerate([
    ...
    txt(s, t, 4.45, 3.8+i*0.52, 3.6, 0.5, sz=12, color=C_DARK)

# ROC by sex images
txt(s, "M1  by Sex", 8.2, 3.3, 2.4, 0.28, sz=11, bold=True)
pic(s, img(M1,"test_roc_by_sex.png"), 8.2, 3.6, 2.4, 1.9)
txt(s, "M3  by Sex", 10.75, 3.3, 2.4, 0.28, sz=11, bold=True, color=C_ORG)
pic(s, img(M3N,"test_roc_by_sex.png"), 10.75, 3.6, 2.4, 1.9)
txt(s, "△ 여성 AUC 0.792 → 0.876  |  △ 남성 AUC 0.882 → 0.900",
    8.2, 5.55, 5.0, 0.3, sz=11, bold=True, color=C_DARK, align=PP_ALIGN.CENTER)

# Training curves
pic(s, img(M3N,"training_curves.png"), 8.2, 5.9, 4.9, 1.35)
txt(s, "M3 norm — Training Curves", 8.2, 5.83, 4.9, 0.28, sz=10, color=C_MGR)
snum(s, 13)
...
for i,(col,title,items) in enumerate([
    ...
    card(s, 0.3, 1.2+i*1.48, 12.7, 1.41)
    box(s, 0.3, 1.2+i*1.48, 0.12, 1.41, fill=col)
    txt(s, title, 0.55, 1.27+i*1.48, 12.1, 0.36, sz=14, bold=True, color=col)
    for k,item in enumerate(items[:2]):
        txt(s, f"  • {item}", 0.55, 1.67+i*1.48+k*0.32, 6.0, 0.3, sz=11.5, color=C_DARK)
    for k,item in enumerate(items[2:]):
        txt(s, f"  • {item}", 6.8,  1.67+i*1.48+k*0.32, 6.1, 0.3, sz=11.5, color=C_DARK)
snum(s, 14)
...
card(s, 0.3, 1.2, 5.9, 5.7)
txt(s, "연구 제한점", 0.5, 1.28, 5.5, 0.36, sz=16, bold=True, color=C_RED)
hline(s, 0.5, 1.66, 5.5, color=C_RED)
for i,(t,b) in enumerate([
    ...
    txt(s, f"  ▶ {t}", 0.5, 1.78+i*1.0, 5.5, 0.34, sz=13, bold=True)
    txt(s, f"    {b}", 0.5, 2.12+i*1.0, 5.5, 0.58, sz=11.5, color=C_MGR)
card(s, 6.5, 1.2, 6.5, 5.7)
txt(s, "향후 연구 방향", 6.7, 1.28, 6.1, 0.36, sz=16, bold=True, color=C_BLUE)
hline(s, 6.7, 1.66, 6.1, color=C_BLUE)
for i,(t,b) in enumerate([
    ...
    txt(s, f"  ◆ {t}", 6.7, 1.78+i*1.0, 6.1, 0.34, sz=13, bold=True, color=C_BLUE)
    txt(s, f"    {b}", 6.7, 2.12+i*1.0, 6.1, 0.58, sz=11.5)
box(s, 0.3, 7.05, 12.7, 0.35, fill=C_DARK)
txt(s, "감사합니다  |  Questions & Discussion",
    0.3, 7.07, 12.7, 0.3, sz=14, bold=True, color=C_WHT, align=PP_ALIGN.CENTER)
snum(s, 15)
...
def condition_slide(loss, aec_len, variant, sn):
    ...
    s = add_slide()
    box(s, 0, 0, 13.33, 1.05, fill=C_DARK)
    box(s, 0, 0, 0.35, 1.05, fill=col)
    txt(s, f"{LOSS_LBL[loss]}  +  {AEC_LBL[aec_len]}  +  {variant}",
        0.45, 0.08, 10.5, 0.5, sz=22, bold=True, color=C_WHT)
    txt(s, "M2 (Matched) · M3 (Clinic+Scanner+AEC)  |  Brier → Calibration 포함",
        0.45, 0.62, 10.5, 0.34, sz=13, color=C_MGR)
    ...
    for j,(lbl,lc) in enumerate([
        ...
        box(s, cx[j], 1.1, cw[j], 0.33, fill=lc)
        txt(s, lbl, cx[j]+0.05, 1.12, cw[j]-0.1, 0.29,
            sz=11, bold=True, color=C_WHT, align=PP_ALIGN.CENTER)

    pic(s, cmp_png,                        cx[0], 1.45, cw[0], 2.85)
    pic(s, img(m2_dir, "calibration.png"), cx[1], 1.45, cw[1], 2.85)
    pic(s, img(m3_dir, "calibration.png"), cx[2], 1.45, cw[2], 2.85)
    ...
    for j,(lbl,lc) in enumerate([
        ...
        box(s, cx[j], 4.38, cw[j], 0.33, fill=lc)
        txt(s, lbl, cx[j]+0.05, 4.40, cw[j]-0.1, 0.29,
            sz=11, bold=True, color=C_WHT, align=PP_ALIGN.CENTER)

    pic(s, img(m2_dir, "confusion_matrices.png"), cx[0], 4.73, cw[0], 2.62)
    pic(s, img(m3_dir, "confusion_matrices.png"), cx[1], 4.73, cw[1], 2.62)
    pic(s, img(m2_dir, "attention_heatmap.png"),  cx[2], 4.73, cw[2], 2.62)
...
# Dataset comparison
card(s, 0.3, 1.2, 5.5, 3.0)
txt(s, "데이터셋 비교", 0.5, 1.28, 5.1, 0.3, sz=14, bold=True)
hline(s, 0.5, 1.6, 5.1, color=C_BLUE)
cx = [0.4, 1.95, 3.5, 5.0]; cw = [1.5, 1.5, 1.45, 0.9]
trow_h(s, ["구분","OK","OK+Missing","차이"], cx, cw, 1.65, ht=0.3)
for i,(row,fill) in enumerate(zip([
    ("전체 n",    "859",  "1,162", "+303"),
    ("Train n",   "687",  "929",   "+242"),
    ("Test n",    "172",  "233",   "+61"),
    ...
    trow_d(s, row, cx, cw, 1.97+i*0.37, ht=0.37, fill=fill)

# Performance comparison table
card(s, 0.3, 4.35, 5.5, 2.85)
txt(s, "BCE+AEC128+norm 성능 비교", 0.5, 4.43, 5.1, 0.3, sz=14, bold=True)
hline(s, 0.5, 4.75, 5.1, color=C_ORG)
px = [0.4, 1.65, 2.85, 4.15]; pw = [1.2, 1.15, 1.25, 1.35]
trow_h(s, ["모델","OK AUC","OM AUC","Δ"], px, pw, 4.8, ht=0.3)
for i,(row,fill) in enumerate(zip([
    ...
    d_col = C_RED  # all decreases
    trow_d(s, row, px, pw, 5.12+i*0.37, ht=0.37, fill=fill,
           colors=[C_DARK,C_DARK,C_DARK,d_col])

# Key findings
card(s, 6.0, 1.2, 7.1, 3.4, fill=RGBColor(0xFF,0xF3,0xE0))
box(s, 6.0, 1.2, 0.12, 3.4, fill=C_ORG)
txt(s, "주요 발견", 6.25, 1.28, 6.7, 0.3, sz=14, bold=True, color=C_ORG)
for i,t in enumerate([
    ...
    txt(s, t, 6.25, 1.68+i*0.43, 6.7, 0.4, sz=12, color=C_DARK)

# DeLong summary table
card(s, 6.0, 4.72, 7.1, 2.48)
txt(s, "DeLong 요약  (norm 기준 / ok+missing)", 6.2, 4.8, 6.8, 0.3, sz=13, bold=True)
hline(s, 6.2, 5.12, 6.6, color=C_TEAL)
dx2 = [6.1, 7.6, 8.8, 10.1, 11.3]; dw2 = [1.45,1.15,1.25,1.15,1.65]
trow_h(s, ["비교","OK p","OM p","OK sig","OM sig"], dx2, dw2, 5.17, ht=0.28)
dlong_om_rows = [
...
for i,(lbl,ok_p,om_p,ok_sig,om_sig,ok_col,om_col) in enumerate(dlong_om_rows):
    ...
    trow_d(s, [lbl, ok_p, om_p, ok_sig, om_sig],
           dx2, dw2, 5.47+i*0.37, ht=0.37, fill=fill,
...
def cond_slide_om(loss, aec_len, sn):
    ...
    txt(s, f"[OK+Miss] {LOSS_LBL[loss]}  +  {AEC_LBL[aec_len]}  — 간략 요약",
        0.45, 0.08, 10.5, 0.5, sz=21, bold=True, color=C_WHT)
    txt(s, "OK+Missing 코호트 (n=1,162)  |  norm · excl_extreme 조건 비교 / DeLong 전 조건 ns",
        0.45, 0.62, 10.5, 0.34, sz=12, color=C_MGR)
    txt(s, f"{sn} / {TOTAL}", 12.3, 7.15, 1.0, 0.3, sz=11,
        color=C_MGR, align=PP_ALIGN.RIGHT)

    # Left: 2 ROC panels stacked
    for j,(png,lbl,y0) in enumerate([
        (cmp_png_n, "ROC — norm 조건", 1.1),
        (cmp_png_e, "ROC — excl_extreme 조건", 4.28),
    ]):
    ...
    calib_banner(s, 6.4, 1.1, 6.7, f"M2  {LOSS_LBL[loss]}+{AEC_LBL[aec_len]}+norm  (Brier)")
    pic(s, img(m2_dir, "calibration.png"), 6.4, 1.45, 6.7, 2.8)
    calib_banner(s, 6.4, 4.38, 6.7, f"M3  {LOSS_LBL[loss]}+{AEC_LBL[aec_len]}+norm  (Brier)")
    pic(s, img(m3_dir, "calibration.png"), 6.4, 4.73, 6.7, 2.65)
