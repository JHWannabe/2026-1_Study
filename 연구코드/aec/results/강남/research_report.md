...
|---------|-----------|------|-----|
| **mean** | +0.297 | AEC 평균값 → 전체 tube current 수준 | 1.53 |
| **CV** | -0.349 | 변동계수 (std/mean) → 체형 불균일성 반영 | 1.58 |
| **skewness** | -0.343 | AEC 곡선 비대칭성 → 체형 분포 특성 | 1.49 |
| **slope_abs_mean** | +0.042 | 평균 절대 기울기 → 곡선 동역학 | 1.42 |
...
|------|----|------|
| N | 1673 | 총 환자 수 |
| R² | **0.6577** | TAMA 분산의 65.8% 설명 |
| Adjusted R² | **0.6502** | 변수 수 페널티 적용 설명력 |
| F-statistic | 87.321 (p=0.00e+00) | 전체 모델 유의성 |
| RMSE | **17.835 cm²** | 예측 오차 (임상 단위) |
| MAE | 13.211 cm² | 이상치 강건 오차 |
| AIC | 14462.20 | 모델 복잡도 대비 적합도 |
| BIC | 14662.83 | 강화된 복잡도 페널티 |
...
|------|--------|---------|------|------|
| Shapiro-Wilk | 0.9596 | 0.00e+00 | 비정규 | 잔차 정규성 |
| Breusch-Pagan | 122.8969 | 0.00e+00 | 이분산 | 등분산성 |
| Durbin-Watson | 1.9769 | — | 정상 | 자기상관 (1.5-2.5 정상) |
| Condition number (κ) | 139.85 | — | 주의(κ<1000) | 다중공선성 |
...
| AUC-ROC | **0.7514** [0.7258–0.7775] | 판별 능력 (Bootstrap 95%CI) |
| Sensitivity | 0.7350 | TP/(TP+FN) |
| Specificity | 0.6520 | TN/(TN+FP) |
| PPV | 0.3989 | 양성 예측도 |
| NPV | 0.8868 | 음성 예측도 |
| Hosmer-Lemeshow | χ²=6.410, p=0.601 | 보정도 (p>0.05 = 보정 양호) |
| Nagelkerke R² | **0.2135** | 로지스틱 설명력 (pseudo-R²) |
| Brier Score | 0.1557 | 확률 예측 정밀도 (0=완벽) |
| AIC | 1657.30 | 모델 복잡도 대비 적합도 |
...
|------|-------------------|------------------|---------------|------------------|--------------|--------------|
| R² | 0.1909 | 0.5507 | 0.6356 | 0.6597 | **+0.0849** | +0.0241 |
| Adj R² | 0.1889 | 0.5502 | 0.6343 | 0.652 | +0.0841 | +0.0177 |
| RMSE (cm²) | 27.4216 | 20.4337 | 18.4019 | 17.784 | -2.0318 | -0.6179 |
| AIC | 15837.48 | 14849.28 | 14506.83 | 14454.56 | -342.45 | -52.27 |

#### 4.3.2 로지스틱 회귀 성능 비교

| 지표 | Case 0 (AEC 단독) | Case 1 (Sex+Age) | Case 2 (+AEC) | Case 3 (+CT/kVp) | Case 1→2 향상 | Case 2→3 향상 |
|------|-------------------|------------------|---------------|------------------|--------------|--------------|
| AUC | 0.6786 | 0.624 | 0.7191 | 0.7542 | **+0.0951** | +0.0351 |
| Nagelkerke R² | 0.0976 | 0.0516 | 0.1587 | 0.2159 | +0.1071 | +0.0572 |
| AIC | 1737.82 | 1787.8 | 1667.18 | 1656.25 | -120.62 | -10.93 |
| HL p | 1.3192e-01 | 1.2590e-03 | 3.8738e-01 | 7.2822e-01 | — | — |
...
2. **AEC 단독 진단력 (Case 0):** 성별·나이 없이 AEC 특징만으로 선형 R²=0.191, 로지스틱 AUC=0.679. AEC 신호가 독립적인 근육량 정보를 내포함을 시사.

3. **AEC 특징의 추가 기여 (Case 1→2):** AEC feature 추가 시 선형 R²가 **0.551 → 0.636** (Δ+0.085)으로 향상됨. 로지스틱 AUC는 **0.624 → 0.719** (Δ+0.095).

4. **CT 모델명·kVp의 추가 기여 (Case 2→3):** 모델명/kVp 추가 시 선형 R²가 추가로 +0.024, 로지스틱 AUC +0.035 향상. AIC 감소로 모델 적합도 개선 확인.

5. **최종 모델 (Case 3):** 선형 R²=0.6597, RMSE=17.784 cm², 로지스틱 AUC=0.7542 [0.7289-0.78].
