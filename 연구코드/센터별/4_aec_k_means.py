...
OUTPUT_DIR = BASE_DIR / "result/0327" / SITE_NAME / "4_aec_k_means_train_test"
...
def save_sex_ratio_plot(df: pd.DataFrame, split_name: str, output_path: Path) -> None:
    ratio_df = (
    ...
    ax.set_title(f"{split_name} Sex Ratio by Cluster")
    ax.legend(title="Sex")
    ax.set_ylim(0, 100)
    fig.tight_layout()
    fig.savefig(output_path, dpi=200)
...
def save_boxplot(df: pd.DataFrame, column: str, title: str, output_path: Path) -> None:
...
def save_split_outputs(
    ...
    save_sex_ratio_plot(
        df, split_name, output_dir / f"{split_prefix}_cluster_sex_ratio.png"
    )
    save_boxplot(
        df,
        "BMI",
        f"{split_name} BMI Distribution by Cluster",
        output_dir / f"{split_prefix}_cluster_bmi_boxplot.png",
    )
    save_boxplot(
        df,
        "TAMA",
        f"{split_name} TAMA Distribution by Cluster",
        output_dir / f"{split_prefix}_cluster_tama_boxplot.png",
    )
    ...
    split_df = pd.concat(
        [
            train_df.assign(split="train")[
                ["PatientID", "cluster_id", "image_path", "split"]
            ],
...
def main() -> None:
    ...
    split_info = pd.concat(
        [
            train_df.assign(split="train")[
                ["PatientID", "PatientSex", "BMI", "TAMA", "split"]
            ],
            test_df.assign(split="test")[
                ["PatientID", "PatientSex", "BMI", "TAMA", "split"]
            ],
    ...
    save_metrics_report(train_df, test_df, train_metrics, test_metrics, OUTPUT_DIR)
