...
def configure_sex_filter(sex_filter: str) -> None:
    global SEX_FILTER, SEX_FILTER_DIR, OUTPUT_DIR, AGE_GROUP
...
def load_clinical_data() -> pd.DataFrame:
    usecols = CLINICAL_USECOLS.copy()
    df_all = pd.read_excel(EXCEL_PATH).copy()
...
def save_dendrogram(
    feature_matrix: np.ndarray, n_clusters: int, output_dir: Path = OUTPUT_DIR
) -> None:
    linkage_matrix = linkage(feature_matrix, method="ward")
    cut_height = None
    if 1 < n_clusters < len(feature_matrix):
        cut_height = float(linkage_matrix[-(n_clusters - 1), 2])
    ...
    ax.set_title("AEC Hierarchical Clustering Dendrogram")
    ax.set_xlabel("Samples")
    ax.set_ylabel("Ward Distance")
    ax.grid(True, linestyle="--", alpha=0.3, axis="y")
    fig.tight_layout()
    fig.savefig(output_dir / "aec_dendrogram.png", dpi=200)
    plt.close(fig)
...
def save_analysis_outputs(
    ...
    save_dendrogram(feature_matrix, n_clusters, output_dir=output_dir)
    save_silhouette_plot(scores_df, output_dir=output_dir)
    save_csv_with_fallback(scores_df, output_dir / "silhouette_scores.csv")
    save_detailed_silhouette_plot(
        dataset_df, feature_matrix, n_clusters, output_dir=output_dir
    )
    save_cluster_mean_aecs(clustered_df, output_dir=output_dir)
    remove_output_file(output_dir, "cluster_sex_ratio.png")
    remove_output_file(output_dir, "cluster_bmi_boxplot.png")
    remove_output_file(output_dir, "cluster_tama_boxplot.png")
    remove_output_file(output_dir, "cluster_members.csv")
    save_summary_tables(clustered_df, scores_df, output_dir=output_dir)
    save_report(clustered_df, scores_df, n_clusters, output_dir=output_dir)
...
def run_single_sex_filter() -> None:
    ...
    clustered_df = cluster_aecs(dataset_df, feature_matrix, n_clusters)
